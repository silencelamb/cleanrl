...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = RectangleGridOrderEnv(use_image=True)
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        with torch.no_grad():
            n_flatten = self.cnn1(torch.as_tensor(envs.single_observation_space.sample()[None]).float()).shape[1]

        self.linear1 = nn.Sequential(layer_init(nn.Linear(n_flatten, features_dim)), nn.ReLU())
        ...
        # Compute shape by doing one forward pass
        with torch.no_grad():
            n_flatten = self.cnn2(torch.as_tensor(envs.single_observation_space.sample()[None]).float()).shape[1]

        self.linear2 = nn.Sequential(layer_init(nn.Linear(n_flatten, features_dim)), nn.ReLU())
        
        self.actor = nn.Sequential(
            self.cnn1,
            self.linear1,
            layer_init(nn.Linear(features_dim, envs.single_action_space.n), std=0.01),
        )
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs, action_masks)
